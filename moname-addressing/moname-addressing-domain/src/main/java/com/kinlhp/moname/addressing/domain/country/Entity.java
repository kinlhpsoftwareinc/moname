...
public enum Entity implements Identifiable<String> {
	...
	public static String numericCodeOf(final int numericCode) {
		return "%03d".formatted(numericCode);
	}

	public static int numericCodeOf(@Nonnull final String numericCode) {
		return Integer.parseInt(numericCode);
	}

	@Nonnull
	private static RuntimeException throwException(@Nonnull final String numericCode) {
		@Nonnull final var message = "There is no country identified by numeric code %s".formatted(numericCode);
		LOG.error(message);
