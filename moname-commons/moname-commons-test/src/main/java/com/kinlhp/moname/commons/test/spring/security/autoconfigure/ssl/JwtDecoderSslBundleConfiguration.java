...
@Profile("test & security & keycloak-optimized")
public class JwtDecoderSslBundleConfiguration {
	...
	JwtDecoderSslBundleConfiguration(
			...
			@Nullable @Value("${server.ssl.bundle}") final String bundleName) {
		...
		this.bundleName = Objects.requireNonNullElse(bundleName, "ed25519-p12");
	}
	...
	private RestOperations sslBundleRestOperations() {
		...
		@Nonnull final var requestFactorySettings = ClientHttpRequestFactorySettings.defaults()
				.withSslBundle(sslBundle())
				.withConnectTimeout(JWK_SET_CONNECT_TIMEOUT)
				.withReadTimeout(JWK_SET_READ_TIMEOUT);
		@Nonnull final var requestFactory = ClientHttpRequestFactoryBuilder.detect().build(requestFactorySettings);
	...
	private SslBundle sslBundle() {
		return Optional.ofNullable(sslBundles)
				...
				.orElseGet(() -> {
					LOG.warn("No one SSLBundle named {} was found, using system default SSLBundle", bundleName);
