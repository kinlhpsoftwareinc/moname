...
final class ClientCredentialsFlowExtensionPlatformTestKitFailedIT
		extends AbstractClientCredentialsFlowExtensionPlatformTestKitIT {

	@Nonnull
	private static Stream<Class<? extends ClassToBeTested>> factoryMethod() {
		return Stream.of(ClientCredentialsFailedIT.class);
	}

	@Override
	protected void assertTestEvents(@Nonnull final Events events) {
		org.junit.jupiter.api.Assertions.assertAll(
				() -> events.assertStatistics(statistics -> statistics.succeeded(0L)),
				() -> Assertions.assertThat(events.failed().list())
						.allMatch(event ->
								event.getRequiredPayload(TestExecutionResult.class)
										.getThrowable().orElseThrow()
										.getMessage().contains(FAILED_MESSAGE))
		);
	}
	...
	static final class ClientCredentialsFailedIT implements ClassToBeTested {

		private static final String CLIENT_ID = "failed@" + TWICE_EXECUTION_WORKAROUND_PROPERTY_KEY;
		...
		public void methodToBeTested(@Nonnull final CapturedOutput output,
			...
			Assertions.fail(FAILED_MESSAGE);
		}
	}
}
