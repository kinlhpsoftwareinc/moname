...
final class StreamableRepositoryTests<T extends Identifiable<Integer>> {
	...
	@Test
	void of() {
		Assertions.assertNotNull(StreamableRepository.of(Stream::empty));
	}
	...
	@Test
	void filter() {
		@Nonnull final var result = repository.filter(element -> element.is(0) || element.is(10));
		Assertions.assertEquals(1, result.count());
	}

	@Test
	void collect() {
		@Nonnull final var collector = SingletonCollector.toSingle();
